...
impl Solution {
    ...
    proof fn ways_mod_bounds(n: int, kind: int)
        ensures
            0 <= Self::ways_mod(n, kind) < 1_000_000_007,
        decreases n,
    {
        if n <= 0 {
        } else if n == 1 {
        } else {
            Self::ways_mod_bounds(n - 1, 0);
            Self::ways_mod_bounds(n - 1, 1);
        }
    }
            ...
            res as int == (Self::ways_mod(n as int, 0) + Self::ways_mod(n as int, 1)) % 1_000_000_007,
    {
        ...
        while i < n
            invariant
                ...
                0 <= a121 < modv,
                0 <= a123 < modv,
                a121 as int == Self::ways_mod(i as int, 0),
                a123 as int == Self::ways_mod(i as int, 1),
            decreases n - i,
        {
        ...
        proof {
            Self::ways_mod_bounds(n as int, 0);
            Self::ways_mod_bounds(n as int, 1);
        }
